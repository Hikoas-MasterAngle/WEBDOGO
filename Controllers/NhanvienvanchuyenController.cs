...
namespace WEBDOGO.Controllers
{
    public class NhanvienvanchuyenController : BaseController
    {
        ...
        public ActionResult chinhsuavanchuyenchitietsanpham(VANCHUYEN vanchuyen1)
        {
            ...
            {
                ViewBag.nhanvienvc = db.NHANVIENVANCHUYENs.ToList();
                return View(vanchuyen1);
            }
            var ngayDukienDuocGiao = db.VANCHUYENs
                .Where(m => m.MAVANCHUYEN == vanchuyen1.MAVANCHUYEN)
                .Select(m => m.NGAYDUKIENDUOCGIAO)
                .FirstOrDefault();

            var ngayBatDauGui = db.VANCHUYENs
                .Where(m => m.MAVANCHUYEN == vanchuyen1.MAVANCHUYEN)
                .Select(m => m.NGAYBATDAUGUI)
                .FirstOrDefault();
            if (vanchuyen1.NGAYDUKIENDUOCGIAO == null)
            {
                vanchuyen1.NGAYDUKIENDUOCGIAO = ngayDukienDuocGiao;
            }

            if (vanchuyen1.NGAYBATDAUGUI == null)
            {
                vanchuyen1.NGAYBATDAUGUI = ngayBatDauGui;
            }
            if(vanchuyen1.NGAYDUKIENDUOCGIAO == null && vanchuyen1.NGAYBATDAUGUI == null) {
                vanchuyen1.NGAYDUKIENDUOCGIAO = ngayDukienDuocGiao;
                vanchuyen1.NGAYBATDAUGUI = ngayBatDauGui;
            }
            db.Entry(vanchuyen1).State = EntityState.Modified;
            db.SaveChanges();
